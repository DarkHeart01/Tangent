...
from providers.base import CompletionResult, EmbeddingProvider, LLMProvider, StreamChunk, ToolCall
...
class GeminiAdapter(LLMProvider, EmbeddingProvider):
    ...
    Supports tool-calling, streaming, exponential-backoff retry on rate limits,
    and text embeddings (used by the code indexer).

    No prompt-caching support here (unlike providers.openrouter): Gemini's
    context-caching API has a different request shape and is out of scope.
    """
    ...
    async def embed(self, texts: list[str], model: str = _DEFAULT_EMBEDDING_MODEL) -> list[list[float]]:
        if not texts:
            return []
        ...
        try:
            ...
            if response.status_code == 429:
                raise RateLimitError(f"Gemini rate limit: {response.text}")
            if response.status_code != 200:
                raise ProviderError(f"Gemini embeddings error {response.status_code}: {response.text[:400]}")

            data = response.json()
        except (RateLimitError, ProviderError):
            raise
        except Exception as exc:
            log.error("gemini_embedding_error", model=model, error=str(exc))
            raise ProviderError(str(exc)) from exc
