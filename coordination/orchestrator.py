...
class SwarmRuntime:
    ...
    def __init__(
        ...
        code_index_builder: Optional[IndexBuilder] = None,
        indexing_top_k: int = 6,
        on_event: Optional[Callable[[dict], None]] = None,
        on_gate_request: Optional[Callable[[str, str], Awaitable[bool]]] = None,
    ) -> None:
        ...
        self._code_searcher = code_searcher
        self._code_index_builder = code_index_builder
        self._indexing_top_k = indexing_top_k
    ...
    def _emit(self, event_type: str, **detail: Any) -> None:
        if self._on_event is None:
            return
        try:
            top = {k: v for k, v in detail.items() if k in self._EVENT_TOP_LEVEL_KEYS}
            rest = {k: v for k, v in detail.items() if k not in self._EVENT_TOP_LEVEL_KEYS}
            self._on_event({"type": event_type, "run_id": self.trace_id, **top, "detail": rest})
        except Exception:
            log.warning("on_event_failed", event_type=event_type)
    ...
    def _wire_tools(self) -> None:
        ...
        if self._code_index_builder:
            cib_h.set_builder(self._code_index_builder)
    ...
    async def _spawn_agent_for_goal(
        ...
        iteration_bonus: float = 1.0,
    ) -> TaskResult:
        agent = self._make_agent(role)
        if iteration_bonus != 1.0:
            bumped = max(1, int(agent.spec.termination.max_iterations * iteration_bonus))
            agent.spec = agent.spec.model_copy(update={
                "termination": agent.spec.termination.model_copy(update={"max_iterations": bumped})
            })
    ...
    async def run(self, goal: str) -> TaskResult:
        ...
        self._emit("run_start", goal=goal[:200])

        if self._code_index_builder:
            try:
                stats = await self._code_index_builder.build()
                log.info("code_index_ready", files_indexed=stats.files_indexed,
                         files_skipped=stats.files_skipped, chunks_total=stats.chunks_total)
            except Exception as exc:
                log.warning("code_index_build_failed", error=str(exc))

        strategy = self.topology.coordination.strategy

        # ── Lifecycle-aware mode (§20) ────────────────────────────────────────
        if strategy == "lifecycle":
            result = await self._run_lifecycle(goal)
            self._emit("run_end" if result.success else "run_error",
                       success=result.success, cost_usd=self._ledger.total_cost)
            return result
        ...
        if len(available_roles) <= 1:
            ...
            agent = self._make_agent(role)
            result = await agent.run_task(root_task, trace_id=self.trace_id)
            self._emit("run_end" if result.success else "run_error",
                       success=result.success, cost_usd=self._ledger.total_cost)
            return result
        ...
        self._emit("run_end" if final_result.success else "run_error",
                   success=final_result.success, cost_usd=self._ledger.total_cost)
        return final_result
    ...
    async def _run_lifecycle(self, goal: str) -> TaskResult:
        ...
        while current_phase_idx < len(phases):
            ...
            self._emit("phase_start", phase_id=phase_id, phase_name=phase_name)
            ...
            self._emit("phase_end", phase_id=phase_id, phase_name=phase_name,
                       success=phase_success, output_missing=output_missing)
            ...
            if has_gate and approval_gates and safety_mode == "interactive":
                ...
                self._emit("phase_gate_resolved", phase_id=phase_id, approved=approved)
                if not approved:
    ...
    async def _run_phase_agents(self, agent_roles: list[str], phase_task: Task) -> list[TaskResult]:
        ...
        An agent that runs out of iterations without producing a final answer
        (MaxIterationsError) is retried once with a 50% larger iteration budget —
        it's usually mid-task rather than stuck, so a bigger ceiling often clears
        it. Other errors (tool failures, provider errors, etc.) are not retried,
        since re-running from scratch wouldn't fix them.
        """
        import asyncio
        ...
        for role in agent_roles:
            ...
            try:
                ...
                log.warning("phase_agent_iteration_exhausted_retrying", role=role, phase=phase_id)
                try:
                    result = await self._spawn_agent_for_goal(
                        role, goal,
                        timeout=phase_task.constraints.timeout,
                    ...
                    log.error("phase_agent_error", role=role, error=str(exc), retried=True)
