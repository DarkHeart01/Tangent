...
class TerminationPolicy(BaseModel):
    ...
    max_tokens: int = 8192
    # Context-window budget that triggers scratchpad compaction — distinct from
    # max_tokens above, which caps a single completion's *output* tokens.
    context_window_tokens: int = 60000
    # Max characters kept per string field in a tool result before it's
    # truncated on the way into message history.
...
class AgentSpec(BaseModel):
    ...
    code_context: bool = False   # auto-inject top-k code-index search results for task.goal
...
class IndexingConfig(BaseModel):
    """Code-index settings — semantic search over the local repo for coding agents."""
    enabled: bool = True
    embedding_model: str = "text-embedding-004"
    chunk_lines: int = 200
    chunk_overlap: int = 20
    top_k: int = 6
    index_dir: str = ".devforge/index"
    exclude_dirs: list[str] = Field(
        default_factory=lambda: [
            "node_modules", "venv", ".venv", "__pycache__", ".git",
            "dist", "build", ".next", "built", ".ruff_cache", ".mypy_cache",
            ".pytest_cache", "traces", "memory_store", "graphify-out",
        ]
    )
