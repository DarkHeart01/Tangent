...
from indexer.search import CodeIndexSearcher, format_for_prompt, format_repo_overview
...
class Agent:
    ...
    def __init__(
        ...
        self._scratchpad = scratchpad or Scratchpad(
            self.id,
            max_tokens=spec.termination.context_window_tokens,
        )
    ...
    async def run_task(self, task: Task, trace_id: Optional[str] = None) -> TaskResult:
        ...
        with Span(tracer, f"agent.{self.spec.role}", "agent",
            ...
            self._emit("agent_start", task_id=task.id)
            ...
            # Optionally seed a directory-tree/language/dependency overview so
            # coding agents know what exists in the repo before deciding what
            # to read, rather than relying solely on goal-text similarity.
            if self.spec.code_context and (repo_inspect := self._tools.get("repo_inspect")):
                try:
                    overview = await repo_inspect.run({}, agent_id=self.id)
                    overview_block = format_repo_overview(overview)
                    if overview_block:
                        messages[0]["content"] += f"\n\n## Repository overview\n{overview_block}"
                except Exception as exc:
                    log.warning("repo_overview_failed", agent=self.spec.role, error=str(exc))

            # Optionally seed from the local code index — auto-retrieval so
            # coding agents start grounded instead of guessing at repo structure.
            if self.spec.code_context and self._code_searcher:
                try:
                    code_results = await self._code_searcher.search(task.goal, k=self._indexing_top_k)
                    code_block = format_for_prompt(code_results)
                    if code_block:
                        messages[0]["content"] += f"\n\n## Relevant codebase context\n{code_block}"
                except Exception as exc:
                    log.warning("code_context_search_failed", agent=self.spec.role, error=str(exc))

            # From here on, the scratchpad is the single source of truth for the
            # conversation history — it may compact (drop/summarize older
            # messages) as the loop progresses, so re-read it each iteration
            # rather than keeping a separately-growing local list.
            self._scratchpad.seed(messages)
            ...
            while iteration < self.spec.termination.max_iterations:
                ...
                messages = self._scratchpad.get_messages()

                # ── Perceive + Plan: call LLM ──────────────────────────────────
                self._emit("agent_thinking", task_id=task.id, iteration=iteration)
                ...
                if not result.tool_calls:
                    ...
                    self._emit("agent_done", task_id=task.id, cost_usd=task_result.cost)
                    return task_result
                ...
                for tc in result.tool_calls:
                    tool_result_content = await self._execute_tool(tc.name, tc.arguments,
                                                                    tc.id, task)
                    tool_result_content = cap_tool_output(
                        tool_result_content, max_chars=self.spec.termination.tool_output_max_chars,
                    )
            ...
            self._emit("agent_error", task_id=task.id, error="max_iterations_exceeded")
    ...
    async def _execute_tool(
        ...
        try:
            ...
            self._emit("tool_result", task_id=task.id, tool=name, call_id=call_id, ok=True)
            return result
        except Exception as exc:
            log.error("tool_error", agent=self.spec.role, tool=name, error=str(exc))
            task.add_trace("tool_error", tool=name, error=str(exc))
            self._emit("tool_result", task_id=task.id, tool=name, call_id=call_id, ok=False, error=str(exc))
