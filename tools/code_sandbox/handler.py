    ...
    async def _run(self, inputs: dict[str, Any]) -> dict[str, Any]:
        ...
        try:
            ...
            try:
                ...
                # On Windows, running the server with uvicorn's --reload (or
                # --workers > 1) forces WindowsSelectorEventLoopPolicy, which
                # cannot spawn subprocesses at all — every asyncio.create_subprocess_*
                # call raises this (with an empty message) regardless of the
                # command being run. Run the server without --reload/--workers
                # on Windows to get a subprocess-capable event loop.
                return {
                    "error": (
                        "Subprocess execution is unavailable on this event loop "
                        "(Windows + uvicorn --reload/--workers sets a policy that "
                        "can't spawn subprocesses). Restart the server without "
                        "--reload and without --workers > 1."
                    )
                }
            try:
