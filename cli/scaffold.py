...
from configs.schema import TopologySpec
...
def scaffold_topology_from_spec(spec: TopologySpec, examples_dir: str) -> Path:
    """Write a user-built TopologySpec (from the Topology Builder UI) to disk.

    Unlike scaffold_topology() above (a fixed CLI stub template), this
    serializes the actual validated spec so agents/coordination survive the
    round trip through configs.loader.load_topology_spec().
    """
    slug = _slugify(spec.name)
    folder = Path(examples_dir) / slug
    folder.mkdir(parents=True, exist_ok=True)
    path = folder / "topology.yaml"
